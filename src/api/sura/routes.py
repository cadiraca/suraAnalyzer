...
async def get_contracts(api_key: str = Depends(verify_api_key)) -> ContractListResponse:
    """
...
async def save_upload_file(upload_file: UploadFile) -> str:
    ...
    try:
        ...
        suffix = (
            os.path.splitext(upload_file.filename or "")[1]
...
async def analyze_eligibility_stream(
    ...
    contract_id: Optional[str] = Form(
        None, description="Contract ID (optional, uses default if not provided)"
    ),
        ...
        try:
            ...
            for idx, file in enumerate(files):
                if not file.filename:
                    error_event = SSEErrorEvent(
                        error=f"Archivo #{idx + 1} sin nombre", error_code="NO_FILENAME"
                    )
                    yield f"event: error\ndata: {error_event.model_dump_json()}\n\n"
                    return
            ...
            logger.info(
                f"Calling Gemini API for eligibility analysis with streaming..."
            )
            ...
            try:
                ...
                eligibility_schema = (
                    EligibilityAnalysisSchema.get_eligibility_json_schema()
                )
                ...
                async for chunk in client.analyze_multiple_files_structured_stream(
                    ...
                    response_text += chunk
                    logger.info(
                        f"[SSE_ROUTE] Received chunk #{chunk_count}, length: {len(chunk)} characters"
                    )
                ...
                analyzing_event = SSEAnalyzingEvent(
                    message="Procesando resultados del análisis...", progress=95
                )
                yield f"event: analyzing\ndata: {analyzing_event.model_dump_json()}\n\n"
                await asyncio.sleep(0)
                ...
                result_event = {"result": eligibility_result.model_dump()}
            ...
            error_event = SSEErrorEvent(
                error=f"Error inesperado: {str(e)}", error_code="INTERNAL_ERROR"
            )
            yield f"event: error\ndata: {error_event.model_dump_json()}\n\n"
...
async def analyze_file_stream(
    ...
    **Supported File Types:**
    ...
    async def event_stream():
        ...
        try:
            logger.info(
                f"[SSE_ROUTE] Starting file analysis stream for: {file.filename}"
            )
            ...
            if max_tokens < 1 or max_tokens > 8192:
                ...
                logger.error(
                    f"[SSE_ROUTE] Parameter validation failed: max_tokens={max_tokens}"
                )
                yield f"event: error\ndata: {error_data}\n\n"
                return
            ...
            if temperature < 0.0 or temperature > 1.0:
                ...
                logger.error(
                    f"[SSE_ROUTE] Parameter validation failed: temperature={temperature}"
                )
                yield f"event: error\ndata: {error_data}\n\n"
                return
            ...
            status_data = json.dumps(
                {"message": f"Validating file: {file.filename}", "step": "validation"}
            )
            ...
            if not file.filename:
                error_data = json.dumps(
                    {"message": "No filename provided", "error_code": "NO_FILENAME"}
                )
            ...
            status_data = json.dumps(
                {"message": "Processing uploaded file...", "step": "file_processing"}
            )
            ...
            if not validation["valid"]:
                ...
                logger.error(
                    f"[SSE_ROUTE] File validation failed: {validation['errors']}"
                )
                yield f"event: error\ndata: {error_data}\n\n"
                return
            ...
            logger.info(
                f"[SSE_ROUTE] Starting analysis for {validation['file_type']} file"
            )
            yield f"event: status\ndata: {status_data}\n\n"
            ...
            async for chunk in client.analyze_file_stream(
                ...
                chunk_count += 1
                chunk_data = json.dumps({"content": chunk, "chunk_number": chunk_count})
                logger.info(
                    f"[SSE_ROUTE] Sending chunk #{chunk_count} to client, length: {len(chunk)}"
                )
                yield f"event: chunk\ndata: {chunk_data}\n\n"
            ...
            logger.info(
                f"[SSE_ROUTE] Analysis complete, total chunks sent: {chunk_count}"
            )
            yield f"event: complete\ndata: {complete_data}\n\n"
            ...
            error_data = json.dumps({"message": str(e), "error_code": "ANALYSIS_ERROR"})
            yield f"event: error\ndata: {error_data}\n\n"
        finally:
            ...
            if temp_path and os.path.exists(temp_path):
                try:
                    ...
                    logger.warning(
                        f"[SSE_ROUTE] Failed to cleanup temp file {temp_path}: {cleanup_error}"
                    )
    ...
    async def generate_sse_stream():
        ...
        try:
            ...
            try:
                ...
                response_text = await client.analyze_multiple_files_structured(
                    file_parts=file_parts,
                    prompt=contract.eligibility_instructions,
                    response_schema=eligibility_schema,
                )
                ...
                analyzing_event = SSEAnalyzingEvent(
                    message="Procesando resultados del análisis...", progress=90
                )
                yield f"event: analyzing\ndata: {analyzing_event.model_dump_json()}\n\n"
                await asyncio.sleep(0)
