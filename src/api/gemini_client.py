...
class GeminiClient:
        ...
        if self._model is None:
            try:
                self._model = GenerativeModel(
                    proxy_client=self.proxy_client, model_name=self.model_name
                )
    ...
    def chat_with_functions(
        self, message: str, tools: List[Callable], chat_session: Optional[Any] = None
    ) -> str:
        """
    ...
    def get_chat_history(self, chat_session: Any) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for content in chat_session.history:
                ...
                history.append(
                    {"role": content.role, "content": type(part).to_dict(part)}
                )
            return history
    ...
    async def analyze_pdf(self, pdf_path: str, prompt: str = None, **kwargs) -> str:
        """
        ...
        try:
            ...
            # Encode PDF to base64 (offload to thread since it reads from disk)
            base64_pdf = await asyncio.to_thread(encode_pdf_to_base64, pdf_path)
    ...
    async def analyze_pdf_from_url(
        self, pdf_url: str, prompt: str = None, **kwargs
    ) -> str:
        """
        ...
        try:
            # Validate URL format
            if not (
                pdf_url.startswith(("http://", "https://"))
                or pdf_url.startswith("gs://")
            ):
                raise ValueError(
                    f"Invalid URL format. Must start with http://, https://, or gs://"
                )

            # Additional validation for HTTP/HTTPS URLs
            if pdf_url.startswith(("http://", "https://")) and not self._validate_url(
                pdf_url
            ):
    ...
    async def analyze_pdf_with_custom_prompt(
        self, pdf_path: str, custom_prompt: str, **kwargs
    ) -> str:
        """
        ...
        """
        return await self.analyze_pdf(pdf_path, prompt=custom_prompt, **kwargs)

    async def analyze_pdf_from_url_with_custom_prompt(
        self, pdf_url: str, custom_prompt: str, **kwargs
    ) -> str:
        """
        ...
        Returns:
            Response based on the custom prompt
        """
        return await self.analyze_pdf_from_url(pdf_url, prompt=custom_prompt, **kwargs)
    ...
    async def analyze_file(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Analyzing {file_type} file: {file_path} ({validation['size_mb']}MB)"
            )
            ...
            # Encode file to base64 (offload to thread since it reads from disk)
            base64_data = await asyncio.to_thread(
                encode_file_to_base64, file_path, mime_type
            )
            ...
            if response_mime_type:
                ...
                logger.info(
                    f"Using structured output with MIME type: {response_mime_type}"
                )
    ...
    async def analyze_file_stream(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"[STREAMING] Starting async streaming analysis of {file_type} file: {file_path} ({validation['size_mb']}MB)"
            )
            ...
            logger.info(
                f"[STREAMING] File encoded to base64, size: {len(base64_data)} characters"
            )
            ...
            logger.info(
                f"[STREAMING] Generation config - max_tokens: {max_tokens}, temperature: {temperature}"
            )
            ...
            # Generate content with streaming enabled (offload blocking call to thread)
            ...
            while True:
                ...
                if chunk.text:
                    chunk_count += 1
                    logger.info(
                        f"[STREAMING] Yielding chunk #{chunk_count}, length: {len(chunk.text)} characters"
                    )
                    yield chunk.text

            logger.info(
                f"[STREAMING] Successfully completed streaming analysis of {file_type} file: {file_path} (total chunks: {chunk_count})"
            )

        except Exception as e:
            logger.error(
                f"[STREAMING] Error in streaming analysis of file {file_path}: {e}"
            )
            raise
    ...
    async def generate_content_stream(
        ...
        try:
            ...
            logger.info(
                f"[TEXT_STREAMING] Starting text generation stream for prompt: {prompt[:50]}..."
            )
            logger.info(
                f"[TEXT_STREAMING] Generation config - max_tokens: {max_tokens}, temperature: {temperature}"
            )
            ...
            while True:
                ...
                if chunk.text:
                    chunk_count += 1
                    logger.info(
                        f"[TEXT_STREAMING] Yielding text chunk #{chunk_count}, length: {len(chunk.text)} characters"
                    )
                    yield chunk.text

            logger.info(
                f"[TEXT_STREAMING] Successfully completed streaming generation for prompt: {prompt[:50]}... (total chunks: {chunk_count})"
            )
    ...
    def is_healthy(self) -> bool:
        ...
        Verifies that the SAP AI Core proxy client can be initialized
        without making any LLM inference calls.
    ...
    async def analyze_multiple_files_structured(
        self,
        ...
        **kwargs,
        ...
        try:
            ...
            content = [{"role": "user", "parts": [{"text": prompt}, *file_parts]}]
            ...
            logger.info(
                f"Analyzing {len(file_parts)} file(s) with structured JSON output"
            )
            ...
            logger.info(
                f"Successfully completed multi-file structured analysis ({len(response.text)} chars)"
            )
            return response.text
    ...
    async def analyze_multiple_files_structured_stream(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"[STREAMING] Starting multi-file structured streaming analysis for {len(file_parts)} file(s)"
            )
            ...
            generation_config_params["response_schema"] = response_schema
            logger.info(
                f"[STREAMING] Using structured output with MIME type: application/json"
            )
            ...
            # Generate content with streaming enabled (offload blocking call to thread)
            logger.info(
                "[STREAMING] Starting Gemini stream generation for multi-file structured analysis..."
            )
            ...
            while True:
                ...
                if chunk.text:
                    chunk_count += 1
                    logger.info(
                        f"[STREAMING] Yielding structured chunk #{chunk_count}, length: {len(chunk.text)} characters"
                    )
                    yield chunk.text

            logger.info(
                f"[STREAMING] Successfully completed multi-file structured streaming analysis (total chunks: {chunk_count})"
            )

        except Exception as e:
            logger.error(
                f"[STREAMING] Error in multi-file structured streaming analysis: {e}"
            )
            raise
